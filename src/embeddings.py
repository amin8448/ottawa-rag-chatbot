    ...
    def _load_from_cache(self, texts: List[str]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if cache_file.exists():
                ...
                if cached_data.get("model_name") == self.model_name and cached_data.get(
                    "text_count"
                ) == len(texts):
    ...
    def estimate_memory_usage(self, num_texts: int) -> dict:
        ...
        return {
            ...
            "recommended_batch_size": min(
                self.batch_size, max(1, int(1024 * 1024 * 100 / embedding_size))
            ),  # 100MB batches
        }
...
if __name__ == "__main__":
    ...
    embedding_manager = EmbeddingManager(
        model_name="all-MiniLM-L6-v2", cache_dir="data/embeddings/cache"
    )
