...
class OttawaChatbot:
    ...
    def _create_interface(self) -> gr.Blocks:
        ...
        with gr.Blocks(css=custom_css, title="Ottawa City Services Assistant") as interface:
            ...
            with gr.Tab("💬 Ask Questions"):
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        with gr.Row():
                            example_btn1 = gr.Button("Marriage License", variant="secondary", scale=1)
                            example_btn2 = gr.Button("Parking Rules", variant="secondary", scale=1)
                            example_btn3 = gr.Button("Waste Collection", variant="secondary", scale=1)
                            example_btn4 = gr.Button("Fire Safety", variant="secondary", scale=1)
                ...
                with gr.Tab("📊 System Monitor"):
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            refresh_pipeline_btn = gr.Button("Refresh Pipeline", variant="secondary")
                    
                    with gr.Row():
                        with gr.Column():
                            gr.Markdown("### 📋 Recent Questions")
                            recent_questions = gr.DataFrame(
                                headers=["Time", "Question", "Response Status", "Confidence"],
            ...
            # Event handlers
            def process_question(question: str, history: List) -> Tuple[List, str, str, str]:
                ...
                try:
                    ...
                    if not self.pipeline_initialized:
                        error_msg = "⚠️ System not ready. Please check the system status."
                    ...
                    if response_data.get('answer'):
                        ...
                        return history, confidence_html, sources_html, self._get_status_html()
            ...
            submit_btn.click(
                ...
                outputs=[chatbot_display, confidence_display, sources_display, status_display]
            ).then(
                lambda: "",
            ...
            example_btn1.click(lambda: "How do I apply for a marriage license?", outputs=[question_input])
            example_btn2.click(lambda: "What are the parking regulations in downtown Ottawa?", outputs=[question_input])
            example_btn3.click(lambda: "What can I put in my green bin for waste collection?", outputs=[question_input])
            example_btn4.click(lambda: "What are the rules for backyard fires?", outputs=[question_input])
            
            # Admin panel handlers (if enabled)
            if self.enable_admin:
                refresh_stats_btn.click(
                    self._get_usage_stats,
    ...
    def _format_sources(self, sources: List[Dict]) -> str:
        ...
        for i, source in enumerate(sources[:3]):  # Show top 3 sources
            ...
            if url:
                ...
                display_url = url.replace('https://ottawa.ca/', '').replace('https://', '')
    ...
    def _get_usage_stats(self) -> str:
        ...
        if self.session_history:
            confidences = [entry['confidence'] for entry in self.session_history if entry['success']]
